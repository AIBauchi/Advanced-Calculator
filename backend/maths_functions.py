...
def nth_root(x, n):
    ...
    """
    if x < 0 and n % 2 == 0:
        raise ValueError("Nth root is not defined for negative"
                         "numbers with even roots")
    if x == 0:
        return 0
    lower = 0
    upper = max(1, x)
...
def ln(x):
    ...
    Args:
        x (float): The positive number whose natural logarithm
        is to be computed.
...
def log10(x):
    ...
    Args:
        x (float): The positive number whose base 10 logarithm
        is to be computed.
...
def log(x, base):
    ...
    Args:
        x (float): The positive number whose base 10 logarithm
        is to be computed.
        base (int): The base of the logarithm function
    ...
    Example:
        ...
        1.9590413923210936
    """
    if x <= 0 or (base < 0 and type(base) is not int):
        raise ValueError("Logarithm is not defined for non-positive numbers")
    return math.log10(x)
